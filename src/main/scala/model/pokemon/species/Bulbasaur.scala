...
import model.pokemon.move.bytype.grass.{SleepPowder, StunSpore, VineWhip}
...
class Bulbasaur(override protected val levelTracker: LevelTracker)
  ...
  override def getBaseStats: PokemonStats = new PokemonStats(scala.collection.immutable.Map[String, Int](
    PokemonStats.HP_KEY -> Bulbasaur.BASE_HP,
    PokemonStats.ATK_KEY -> Bulbasaur.BASE_ATK,
    PokemonStats.DEF_KEY -> Bulbasaur.BASE_DEF,
    PokemonStats.SPATK_KEY -> Bulbasaur.BASE_SPATK,
    PokemonStats.SPDEF_KEY -> Bulbasaur.BASE_SPDEF,
    PokemonStats.SPD_KEY -> Bulbasaur.BASE_SPD
  ))
  ...
  override def getBaseExpAwarded: Int = 64
}
